...
namespace shader_precompiler {
	...
	/// </summary>
	class GlesVisitor : public shader_precompiler::ast::VisitorBase {
	...
	public:
		GlesVisitor(shader_precompiler::ast::TreeResult from, IDiagnosticReporter& reporter, std::ostream& to) : from(std::move(from)), reporter(reporter), out(to) {};
		GlesVisitor(shader_precompiler::ast::BaseAstProcessor& from, IDiagnosticReporter& reporter, std::ostream& to) : GlesVisitor(std::move(from.processTree()), reporter, to) {};
		void generate();
	};
}